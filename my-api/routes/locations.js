...
router.get('/latest/:userId', async (req, res) => {
  ...
  try {
    // Try to get the latest location, handling both lat/lng and latitude/longitude column names
    // based on what we saw in the users.js update logic
    const sql = `
      SELECT 
        COALESCE(lat, latitude)::float8 AS latitude,
        COALESCE(lng, longitude)::float8 AS longitude,
        heading::float8 AS heading,
        COALESCE(timestamp, updated_at, created_at) AS timestamp,
        username,
        role
      FROM locations
      WHERE user_id = $1
      ORDER BY COALESCE(timestamp, updated_at, created_at) DESC
      LIMIT 1
    `;
    ...
    console.error(`GET /api/locations/latest/${userId} error:`, err.message);
